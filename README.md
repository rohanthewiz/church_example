...
- Vgo (Go Mod) is now used for dependency management.
...
- And don't worry, Docker will come. Let's just get everything jiving smoothly first.  
...
### if installing from apt then maybe we don't need the below - TODO !! Try and see
mkdir ~/libs
cd libs
...
vips -v # check the installed vips version
```
...
### Configuration
- `cp cfg/options.yml.sample cfg/options.yml`
...
```bash
# Clone the repo into a your project directory of choice
cd ~/myprojects
...
- Start the server `./church_example`
...
- The slug of the Main menu should be hardwired to "main-menu". For now manually update that in the DB
- The slug for the Footer menu also should be hardwired to "footer-menu". Also manually update in DB
